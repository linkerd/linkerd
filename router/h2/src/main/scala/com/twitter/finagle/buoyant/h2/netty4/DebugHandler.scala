...
private[h2] class DebugHandler(prefix: String)
  ...
  override def write(ctx: ChannelHandlerContext, obj: Any, p: ChannelPromise): Unit = {
    ...
    log.info(s"$reqid $prefix.write ${ctx.channel} [$objstr]")
    p.addListener(new ChannelFutureListener {
      override def operationComplete(cf: ChannelFuture): Unit = cf.cause match {
        case null => log.info(s"$reqid $prefix.write.complete ${cf.channel} [$objstr] $cf")
        case e => log.info(e, s"$reqid $prefix.write.complete ${cf.channel} [$objstr] $cf")
      }
    })
