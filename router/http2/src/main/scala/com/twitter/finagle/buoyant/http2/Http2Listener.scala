...
object Http2Listener {
  ...
  private def initHttp2Connection(stream: ChannelInitializer[Channel]) =
    new ChannelInitializer[Channel] {
      val isServer = true
      def initChannel(ch: Channel): Unit = {
        val http2Stream = prepChildStream(stream)
        val _ = ch.pipeline.addLast(new Http2Codec(isServer, http2Stream))
      }
    }

  private def prepChildStream(stream: ChannelInitializer[Channel]) =
    new ChannelInitializer[Channel] {
      def initChannel(ch: Channel): Unit = {
  ...
  // where it happens.
  private def initHttp2Stream(pipeline: ChannelPipeline): Unit = {}
}
