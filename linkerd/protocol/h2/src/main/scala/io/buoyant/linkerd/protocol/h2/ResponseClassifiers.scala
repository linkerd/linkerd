...
class RetryableIdempotent5XXConfig extends H2ResponseClassifierConfig {
  def mk: ResponseClassifier = ResponseClassifiers.RetryableIdempotentFailures
}

class RetryableIdempotent5XXInitializer extends H2ResponseClassifierInitializer {
...
class RetryableRead5XXConfig extends H2ResponseClassifierConfig {
  def mk: ResponseClassifier = ResponseClassifiers.RetryableReadFailures
}

class RetryableRead5XXInitializer extends H2ResponseClassifierInitializer {
...
class NonRetryable5XXConfig extends H2ResponseClassifierConfig {
  def mk: ResponseClassifier = ResponseClassifiers.NonRetryableServerFailures
}

class NonRetryable5XXInitializer extends H2ResponseClassifierInitializer {
...
class AllSuccessfulConfig extends H2ResponseClassifierConfig {
  def mk: ResponseClassifier = ??? //ClassifiedRetries.Default
  // TODO: we still need h2 ClassifiedRetries
}

class AllSuccessfulInitializer extends H2ResponseClassifierInitializer {
