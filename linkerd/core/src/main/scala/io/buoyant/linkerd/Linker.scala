...
import io.buoyant.telemetry.{TelemeterInitializer, TelemeterConfig, Telemeter, TwitterMetricsTelemeter}
...
object Linker {
  ...
  case class LinkerConfig(
    ...
    def mk(): Linker = {
      ...
      // Set DefaultStatsReceiver to use our load stats receiver.
    ...
    private[this] def mkTelemeters() = telemetry match {
      case None =>
        val default = new TwitterMetricsTelemeter
        Seq(default)
