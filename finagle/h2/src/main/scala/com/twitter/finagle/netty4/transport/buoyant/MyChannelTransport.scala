...
import com.twitter.finagle.netty4.transport.Netty4Context
 ...
 */
private[finagle] class MyChannelTransport(
  ...
  private[this] val writeCounter = stats.counter("writes")
  private[this] val writeAndFlushCounter = stats.counter("writeAndFlushes")
  private[this] val flushCounter = stats.counter("flushes")
  ...
  def toFuture(op: ChannelFuture): Future[Unit] = {
  ...
  case class WriteItem(msg: Any, done: Promise[Unit])
  ...
  private[this] val flush: Runnable = { () =>
    //    log.error("Running flush")
    var flushed = false
    while (writeQueue.drainTo(writeChunk, MaxFlushSize) > 0) {
      //      log.error(s"drained ${writeChunk.size} items to write")
...
private[finagle] object MyChannelTransport {
  val HandlerName: String = "myFinagleChannelTransport"
}
